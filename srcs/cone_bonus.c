...
/*   Updated: 2021/03/17 20:05:13 by hekang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cy_set		set_cone_var(t_cylinder *cy, t_ray *r, int type)
{
	...
	c.a = vec_dot(r->dir, r->dir) - pow(vec_dot(r->dir, cy->normal), 2) * (m + 1);
...
int				cone_hit(void *obj, t_ray *r, t_hit_record *rec)
{
	...
	normal = vec_unit(vec_sub_apply(vec_add_apply(tmp, cone->origin), p));
