...
/*   Updated: 2021/03/11 15:43:25 by hekang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

t_cy_set		set_cylinder_var(t_cylinder *cy, t_ray *r)
{
	t_cy_set	c;
	t_vec		*oc;
...
int				cylinder_hit_2(void *obj, t_ray *r, t_hit_record *rec)
{
	...
	normal = vec_unit(vec_sub(p, vec_add(cy->origin,
		vec_mul_const(cy->normal, vec_dot(c.cp, cy->normal)))));
