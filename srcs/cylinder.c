...
/*   Updated: 2021/02/26 15:07:04 by hekang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int             cylinder_hit(void *obj, t_ray *r, t_hit_record *rec)
{
    ...
    if (c.discriminant < 0.00001)
        return (FALSE);
    c.root = (-c.b + sqrt(c.discriminant)) / 2 / c.a;
    ...
    {
        rec->color = cy->color;
        rec->color = vec_create(0,0,rec->t);
        return (TRUE);
    }
    return (FALSE);
}
...
int             cylinder_hit_2(void *obj, t_ray *r, t_hit_record *rec)
{
    ...
    {
        // rec->color = cy->color;
        rec->color = vec_create(0,255,0);
        rec->color = vec_create(0,rec->t, 0);
        printf("t : %f\n", rec->t);
        return (TRUE);
    }
    return (FALSE);
...
//     // rec->normal = vec_unit(vec_add(cy->origin, vec_mul_const(cy->normal, vec_dot(c.cp, cy->normal))));
//     rec->normal = vec_unit(vec_sub(rec->p, vec_add(cy->origin, vec_mul_const(cy->normal, vec_dot(c.cp, cy->normal)))));
//     // printf("rec->normal %f %f %f\n", rec->normal->x, rec->normal->y, rec->normal->z);
//     set_face_normal(r, rec, TRUE);
//     // set_face_normal(r, rec, FALSE);
